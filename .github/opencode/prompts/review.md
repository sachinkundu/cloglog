...
This review runs on a local model with a smaller context window than the cloud reviewer. Read selectively — prioritize files the diff imports, files the diff modifies, and files named in CLAUDE.md / docs/ddd-context-map.md as architectural boundaries.
...
Emit a single JSON object matching the schema below. "patch is correct" after thorough verification is a valid and valuable finding. Do not invent problems.
