...
- **`Edit(replace_all=True)` on a SKILL.md silently breaks pin tests that count occurrences in that file.** Several pins assert `body.count(literal) >= N` (e.g. `tests/plugins/test_auto_merge_skill_handles_silent_holds.py` pinned `plugins/cloglog/scripts/auto_merge_gate.py` count ≥ 2). A `replace_all` rename takes the count to 0 silently. Before any `replace_all` on a SKILL.md, grep `tests/plugins/` for `<filename-without-extension>.*count`/`occurrences` patterns to catch counter-based pins.
