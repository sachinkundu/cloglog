...
- **`Path.cwd()` in backend code is a filesystem fingerprint of the launcher, not an invariant — rule has two halves, host-level AND per-PR; the per-PR resolver now has THREE strategies, not two.** The backend may be launched from dev (`/home/sachin/code/cloglog`), prod (`../cloglog-prod`), or eventually Railway. T-255 fixed the **host-level** fallback (new `REVIEW_SOURCE_ROOT` env var + boot log). T-278 added the per-PR resolver `resolve_pr_review_root(event, ...)` with two strategies: branch lookup and host fallback. T-281 prepended a third strategy and made the resolver SHA-aware: **(0) `find_by_pr_url`** — `tasks.pr_url → task.worktree_id → worktrees.id`, the only path that resolves main-agent close-out PRs (the main agent has no worktree row keyed by the close-off branch). **(1) `find_by_branch`** — agent worktree PRs. **(2) host-level fallback.** After whichever path returns a candidate, the resolver compares `git -C <candidate> rev-parse HEAD` against `event.head_sha` and on mismatch materialises a disposable checkout via `git worktree add --detach <main_clone>/.cloglog/review-checkouts/<sha8>-<pr> <head_sha>`; `_review_pr` wraps stages A/B in `try/finally` to remove it via `_remove_review_checkout`. The return type is `PrReviewRoot(path, is_temp, main_clone)` — every caller MUST inspect `is_temp` to know cleanup is needed, and edits that move "the main clone" must update `_REVIEW_CHECKOUT_SUBDIR`, the cleanup path, and `PrReviewRoot.main_clone` together (three-way coupling within `review_engine.py`). Any subprocess that reads files via `-C`/`cwd=` must take its root from a per-request source when one exists, not just the host fallback — otherwise codex/tools see a different tree than the PR's merge target. Note the two sibling repository methods diverge deliberately: `get_worktree_by_branch` filters to `status='online'` (webhook routing needs a live agent), `find_worktree_by_branch_any_status` does not (review resolution only needs a path on disk — a recently-offline worktree still wins over host fallback). Don't unify them. External-fork PRs deliberately fall through to a `review_source_drift` warning rather than attempting `git fetch <fork-url> <sha>` — see `docs/design/two-stage-pr-review.md` §9.6 for the known direction.
...
### Review Engine — opencode & codex invocation
- **`opencode run --pure` means "no external plugins", NOT "no tool access".** Built-in Read/Bash/Edit stay available under `--pure` — verified on 2026-04-23 by running `opencode run --pure --model ollama/gemma4-e4b-32k ... 'Read the first line of AGENTS.md'` which emitted a real Read tool call and returned file contents. Without `--pure`, the default plugin set activates and traps small models in agentic narration loops ("Calling multiple tools to gather context...") so output is tool-call text and never a JSON blob — `src/gateway/review_engine.py::parse_reviewer_output` then fails on every turn. **Always keep `"--pure"` in `OpencodeReviewer._build_args` (`src/gateway/review_loop.py`).** T-268 removed it on the wrong hypothesis that `--pure` blocked file access; PR #193 through PR #195 had zero opencode coverage until T-272 hotfix restored it. Pin test `test_opencode_argv_passes_pure` guards the regression.
- **Small local models (gemma4-e4b-32k) rubber-stamp `:pass:` regardless of prompt framing.** Observed across PRs #190/193/194/195: opencode emits pass verdicts even when codex finds real HIGH severity issues on the same diff, and sometimes a `:pass:` alongside its own `[CRITICAL]` finding (self-contradiction). Prompt engineering alone does not fix this — the model can't hold enough context to defend severity. Treat opencode's `:pass:` as "first-pass checks didn't surface anything"; codex is the severity-gating reviewer. Upgrading to a bigger local model (qwen2.5:32b+ / llama3.3:70b) or a cheap API model for stage A are the real levers. Measure candidates against known-bug PRs (#178 agent-token hole, #191 MCP bearer hole, #187 gateway-owns-no-tables) before choosing. T-274 tracks the agentic-mode investigation.
- **The review body is a side-channel — every formatter that emits it MUST match every predicate that reads it.** `ReviewLoop._reached_consensus` (`src/gateway/review_loop.py`) refuses to short-circuit a `verdict="approve"` turn when any finding has severity `critical`/`high` (PR #190's predicate, after gemma4-e4b-32k emitted `:pass:` + `[CRITICAL]` in the same turn — the same self-contradiction described above). But `_format_review_body` keyed only on `result.verdict`, so it would still emit a `:pass:` body for that contradictory case. T-227's new `latest_codex_review_is_approval` helper reads that body to gate further review — and on webhook replay would silently skip review of a PR the loop predicate had explicitly refused to approve. Fix shipped in T-227 round 2: mirror the loop predicate inside `_format_review_body` so the body's icon prefix always matches the consensus state. The shared severity set lives canonically as `src/gateway/review_loop._SEVERE_SEVERITIES`; `src/gateway/review_engine._SEVERE_SEVERITIES` mirrors it with a comment cross-referencing the loop predicate. When adding a new reader of any structured output another module also writes, grep the writer for every marker the reader's predicate keys on; if the writer branches on a subset of the fields, lift the shared predicate or add the missing branches.
- **`/pulls/{N}/reviews` approval detection must be per-`commit_id`, not PR-wide.** When scoping "has the bot approved this?" against GitHub's review history, filter rows by `commit_id == head_sha` BEFORE picking the latest. Approval of commit A must not silently suppress review of a newly-pushed commit B. The rest of the review pipeline is per-commit — `pr_review_turns` has `UniqueConstraint("pr_url", "head_sha", "stage", "turn_number")`, `ReviewLoop` scopes consensus to `(pr_url, head_sha, stage)`, and `docs/design/two-stage-pr-review.md` §3 says a new push starts both stages from turn 1. T-227 round 1 caught this as MEDIUM. Any new helper that reads `/pulls/{N}/reviews` to drive a review-gate decision MUST take `head_sha` as a required arg and filter as the first step. Empty `head_sha` short-circuits before the HTTP call; legacy rows without `commit_id` are deliberately excluded (they cannot prove approval of a specific commit). Pin: `tests/gateway/test_review_engine.py::TestLatestCodexReviewIsApproval` covers all three regression cells.
- **Prod runs pre-merge code — expect codex round count to saturate before your fix ships.** Any PR that modifies `review_engine.py`, `review_loop.py`, or codex reviewer prompts is being reviewed by prod's PRE-MERGE copy of those files. The current cap (now `MAX_REVIEWS_PER_PR = 5` post-T-227, was `2` pre-T-227) will trip before every review round you'd otherwise expect. Observed on PR #201: after the round 2 fix push, prod's pre-T-227 cap fired ("Review skipped: this PR already has the maximum of 2 bot review sessions") and no review arrived for the final commit. Do NOT use `make promote` as a workaround mid-PR — promote ships `origin/main`, not your branch, so it doesn't help your PR (and may confuse the next PR's reviewer if it shifts the host fallback mid-flight). Call out the expected saturation in the PR description so the human reviewer isn't surprised when the round count flatlines.
...
- **Leak-after-fix: rules restated in multiple docs drift independently.** When a rule appears in several files (primary body + summaries in other docs + test paraphrases), fixing only the primary body leaves the stale summaries as trap doors — a reader trusting any summary over the primary body will re-implement the wrong form. After fixing a primary rule, `grep -rn "<old wording>" docs/ plugins/ | wc -l` must return zero. Ship a **backstop pin test** that asserts *absence* of every wrong phrasing across every file that carries the rule — asserting absence beats asserting presence because it catches the leak-after-fix class automatically. T-270 PR #194 round 3 caught three stale summaries of predicate-3 after round 2 fixed only the primary body; two backstop pin tests now guard that exact regression (`tests/plugins/test_reconcile_skill_structure.py`).
- **Close-off task `worktree_id` is the main agent's UUID, not the target worktree's.** Close-off tasks created by `on-worktree-create.sh` carry the target worktree's UUID in `close_off_worktree_id` (on the SQLAlchemy `Task` model) but `TaskCard` / `TaskResponse` expose only `worktree_id`, which is deliberately set to the main agent's UUID so `get_my_tasks` surfaces the close-off card in the main session. A predicate filtering `get_board` by `worktree_id == <target_wt_id>` is structurally unsatisfiable. Match by `title == f"Close worktree {wt_name}"` — the deterministic template output at `src/board/templates.py:20` — which IS on `TaskCard`. T-270 PR #194 round 1 finding.
...
### Frontend SSE Events
- **New `EventType` values are three-way coupled — backend + frontend hook + types.** `src/shared/events.py::EventType` is the server-side source. But `frontend/src/hooks/useSSE.ts` has a hardcoded `eventTypes` array and `EventSource.addEventListener` is only called for types in that array — a backend event whose type string isn't in `useSSE.ts` is silently dropped at the browser with no error and no fallback. When adding an SSE event: (1) add `NEW_EVENT = "new_event"` to `src/shared/events.py::EventType`, (2) add `"new_event"` to the `eventTypes` array in `useSSE.ts`, (3) add it to the `SSEEvent['type']` union in `frontend/src/api/types.ts` (optional but keeps TS strict). `useBoard.ts` already has a fallback `else { fetchBoard() }` on unknown events so a bespoke handler isn't required unless you want an optimistic update. T-260 demo had to debug this.
...
- **Known-flaky CI test: `tests/board/test_routes.py::test_backlog_returns_tree`.** Failure mode: two tasks created back-to-back can land with colliding `Task.position` values on faster CI hardware (locally interleaving is slower so collisions don't appear). The test asserts the first-created task is first in the backlog response, so a tie-break flip fails the assertion. If this test trips a PR that doesn't touch `src/board/` it is almost certainly not your PR's fault — leave a diagnosis comment pointing at `src/board/repository.py:336` (`order_by(Task.position)`) and ask the user to rerun CI (the bot token does not have `actions:write`). The structural fix belongs in a Board-context worktree: assert-set rather than assert-list, OR have the create endpoint enforce monotonically-increasing positions on conflict. Observed on PR #201 CI run 24849583597; not yet tracked as its own task.
...
- **`showboat verify` reruns every `exec` block on `make quality` — no live-service calls in `exec`.** `scripts/check-demo.sh` (the `demo-check` step of `make quality`) reruns every `uvx showboat exec` block and byte-compares the output to what was captured. Any `exec` that depends on a live service (ollama model availability, external API, transient network, `make prod` not running) will intermittently fail `make quality` for reasons unrelated to the code under review. Verify-safe demos (canonical: `docs/demos/wt-f47-two-stage-review/T-248/`) use filesystem `grep -q` booleans, in-process `python3 -c` parse round-trips, and pre-captured static text only. For "proof the CLI does X" evidence, run the live call **once, out of band**, and embed the captured result as a `showboat note` — never as an `exec` that re-runs under verify. T-272 PR #195 round 2 HIGH.
