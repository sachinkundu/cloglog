...
- **`Path.cwd()` in backend code is a filesystem fingerprint of the launcher, not an invariant — rule has two halves, host-level AND per-PR.** The backend may be launched from dev (`/home/sachin/code/cloglog`), prod (`../cloglog-prod`), or eventually Railway. T-255 fixed the **host-level** fallback (new `REVIEW_SOURCE_ROOT` env var + boot log). T-278 closed the **per-PR** gap: the review engine now calls `resolve_pr_review_root(event, ...)` which prefers the PR's owning worktree via `IWorktreeQuery.find_by_branch` (Agent context OHS) and only falls back to `settings.review_source_root or Path.cwd()` when the worktree is not on this host. Any subprocess that reads files via `-C`/`cwd=` must take its root from a per-request source when one exists, not just the host fallback — otherwise codex/tools see a different tree than the PR's merge target. Note the two sibling repository methods diverge deliberately: `get_worktree_by_branch` filters to `status='online'` (webhook routing needs a live agent), `find_worktree_by_branch_any_status` does not (review resolution only needs a path on disk — a recently-offline worktree still wins over host fallback). Don't unify them.
