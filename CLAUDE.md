...
### Workflow drift / templating

- **Workflow rules live in `plugins/cloglog/templates/AGENT_PROMPT.md`, never inline in launch prompts.** The launch SKILL emits a per-task `task.md` and points the agent at the template + task.md. Any future workflow rule (new MCP tool, new shutdown step, new inbox event class) updates the template once — every future agent inherits it. Hand-copying workflow rules into per-agent prompts caused the 2026-04-30 inbox-path bug (three agents told to tail `/home/sachin/code/cloglog/.cloglog/inbox` — the project root inbox — instead of `<WORKTREE_PATH>/.cloglog/inbox`; resulted in cross-agent talk and 25 minutes of missed operator retries). Pin: `tests/plugins/test_agent_prompt_template_correct_inbox_paths.py`.
- **Inbox path direction is load-bearing.** Tail the **worktree** inbox (`<WORKTREE_PATH>/.cloglog/inbox`) — the backend webhook fan-out delivers `review_submitted` / `pr_merged` / `ci_failed` / operator messages there. Write lifecycle events (`agent_started`, `pr_merged_notification`, `agent_unregistered`, `mcp_unavailable`, `mcp_tool_error`) to the **project root** inbox (`<PROJECT_ROOT>/.cloglog/inbox`) — the supervisor watches that. Collapsing the two paths to one absolute path is the antipattern; the template now codifies the distinction.
...
- **Never use `\$N` inside an unquoted heredoc relayed through an LLM agent.** The SKILL → Bash-tool → bash boundary collapses `\$N` inconsistently across encoders (observed in antisocial 2026-04-30: rendered launch.sh contained `local file="\"; local key="\"` and `local sig="\"`, tripping `unexpected EOF while looking for matching '"'` at exec time). T-353 fix: switch the launch heredoc to **quoted** form (`<< 'EOF'`) so bash performs zero expansion inside, then bake operator-host paths in via post-render `sed -i "s|@@WORKTREE_PATH@@|...|g"`. Rule generalises: any SKILL that templates a shell script via heredoc should default to a quoted delimiter and substitute the dynamic bits with placeholder tokens after the fact. Pin: `tests/plugins/test_launch_skill_renders_clean_launch_sh.py`. Grep token for future incidents: `unexpected EOF while looking for matching '"'`.
...
- **Never `git remote set-url origin` to embed the bot token — push via inline URL instead.** A persistent bot-token origin URL breaks `make promote` (`prod`'s ruleset rejects bot pushes), leaves stale expired tokens in `.git/config`, and leaks the token via `git remote -v`. The right shape is `git push "https://x-access-token:${BOT_TOKEN}@github.com/${REPO}.git" HEAD:<branch>` — bot identity for one push, no config mutation. Set upstream after via `git branch --set-upstream-to=origin/<branch>` so future operator-side `git pull` works without re-auth. Pin: `tests/plugins/test_skills_no_remote_set_url.py`. Observed 2026-04-30: wave-f46-lifecycle-hardening close-wave left the URL mutated; next `make promote` failed against `prod`.
...
- **Every example command in a SKILL.md that touches GitHub must be bot-authenticated end-to-end** — `BOT_TOKEN=$(...)`, `BRANCH=$(git rev-parse --abbrev-ref HEAD)`, `git push "https://x-access-token:${BOT_TOKEN}@github.com/${REPO}.git" "HEAD:${BRANCH}"`, `git fetch origin "${BRANCH}"`, `git branch --set-upstream-to=origin/${BRANCH}`, `GH_TOKEN="$BOT_TOKEN" gh ...`. The inline-URL push form is mandatory — never `git remote set-url origin "https://x-access-token:..."` (T-363: persistent mutation breaks `make promote` against `prod`'s ruleset, strands expired tokens, and leaks the credential through `git remote -v`). The `git fetch origin "${BRANCH}"` step is load-bearing: a raw-URL push does NOT update `refs/remotes/origin/${BRANCH}`, so `--set-upstream-to` would silently fail and leave the local branch with no upstream — every later `git pull` / plain `git push` then errors with "no upstream configured". Saying "use the github-bot skill's flow" in prose is not enough; readers copy the command they see, not the prose around it. Pin tests should assert the bot-authenticated form is *present* (positive substring), not just the unauthenticated form is absent.
