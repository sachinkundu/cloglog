...
uvx showboat exec "$DEMO_FILE" bash \
  'grep -q "scripts/wait_for_agent_unregistered.py" '"$CLOSE_SKILL"' && echo OK || echo FAIL'
...
uvx showboat exec "$DEMO_FILE" bash \
  'grep -q "scripts/wait_for_agent_unregistered.py" '"$RECONCILE_SKILL"' && echo OK || echo FAIL'

uvx showboat note "$DEMO_FILE" \
  "Step 10 — reconcile enumerates Case A (pr_merged), Case B (wedged), Case C (orphaned)."
uvx showboat exec "$DEMO_FILE" bash \
  'grep -q "Case A" '"$RECONCILE_SKILL"' && grep -q "Case B" '"$RECONCILE_SKILL"' && grep -q "Case C" '"$RECONCILE_SKILL"' && echo OK || echo FAIL'
