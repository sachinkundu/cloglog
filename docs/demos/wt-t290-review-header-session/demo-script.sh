#!/usr/bin/env bash
# Demo: T-290 — review body header shows cross-session session counter,
# not intra-session turn index.
#
# Before T-290: every webhook firing rendered `**codex — turn 1/2**` as
# its first review. PR #209 showed two codex reviews both labelled
# `turn 1/2`, and a reader could not tell whether one was session 1
# of 5 or session 4 of 5.
#
# After T-290: the header renders `**codex — session {N}/5**` where N
# = (prior codex sessions on this PR) + 1. The intra-session turn index
# is no longer shown — since T-227 switched to approval-based stop +
# 5-session backstop, the turn index is an implementation detail no
# reader acts on.
#
# Proofs are stdlib-only (import + ast + textwrap) so `uvx showboat verify`
# runs without project deps.
set -euo pipefail
