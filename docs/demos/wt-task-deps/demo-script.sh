#!/usr/bin/env bash
# Demo: agents are now blocked from starting a task whose parent feature
# has upstream feature-dependencies with incomplete tasks. The 409
# response carries a structured task_blocked payload listing every
# blocker. The same guard fires on update_task_status transitions
# into in_progress.
# Called by `make demo`.
