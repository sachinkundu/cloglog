...
Phase 0 — **unblock fresh-host execution** (must land first; fix is mechanical):

0. Replace `python3 -c 'import yaml'` blocks in all five entry points with the grep+sed pattern from `.cloglog/on-worktree-create.sh:88-105`, hoisted into one shared helper:
   - `plugins/cloglog/hooks/worktree-create.sh:35-41`
   - `plugins/cloglog/hooks/quality-gate.sh:39-43`
   - `plugins/cloglog/hooks/protect-worktree-writes.sh:52-72`
   - `plugins/cloglog/hooks/enforce-task-transitions.sh:50-56`
   - `plugins/cloglog/skills/launch/SKILL.md:223-229` (the launcher template; regenerate `.cloglog/launch.sh` so the generated artifact also picks up the fix)
   Add pin tests: (i) zero `import yaml` substrings under `plugins/cloglog/hooks/` and `plugins/cloglog/skills/launch/`; (ii) extend `tests/plugins/test_launch_skill_uses_abs_paths.py:35-103` to assert the template's `_backend_url` block uses the shared helper, not `import yaml`.
