...
0. Replace `python3 -c 'import yaml'` blocks in `plugins/cloglog/hooks/worktree-create.sh:35-41`, `plugins/cloglog/hooks/quality-gate.sh:39-43`, and `plugins/cloglog/hooks/protect-worktree-writes.sh:52-72` with the grep+sed pattern from `.cloglog/on-worktree-create.sh:88-105`. Hoist the parser into a shared helper sourced by all three hooks to prevent re-divergence. Add a pin test that asserts no `import yaml` substring appears in `plugins/cloglog/hooks/`.
