...
- **Use a GitHub ruleset, not classic branch protection.** Classic protection's `restrictions` (push-by-actor) is org-only — on personal repos like `sachinkundu/cloglog` it returns 404, so the "user-only push" clause cannot be expressed via classic protection. Use the rulesets API (or GitHub UI → Settings → Rules) instead.
- Required ruleset shape (asserted by `make verify-prod-protection`):
  - Target: `refs/heads/prod`, enforcement: `active`.
  - Rule `required_linear_history` — no merge commits; `prod` only fast-forwards from `main`.
  - Rule `update` — blocks all direct pushes EXCEPT bypass actors. This is the rule that implements the "user-only push" requirement.
  - Bypass actors: exactly `RepositoryRole admin` (`actor_id=5`), `bypass_mode=always`. On personal repos the admin role IS the owner account, which is the operator. NO `Integration` (App), `Team`, or non-admin `RepositoryRole` actors — those would let agents/bots/teammates bypass the operator-only gate.
  - NO `pull_request` rule — `make promote` is a fast-forward, not a PR target.
  - Recommended (asserted as warnings, not failures): `non_fast_forward` rule (block force-push of admin's own bypassed pushes) and `deletion` rule (block accidental delete).
- `main` keeps its existing protection (PR + CI required). No change to `main`.
...
8. **Apply ruleset on `prod`** in GitHub settings per §3.2 (Settings → Rules → New branch ruleset, NOT Settings → Branches → classic protection — see §3.2 rationale).
   *Verify:* `make verify-prod-protection` exits 0; an attempted `git push origin :prod` (delete) and `git push --force origin abc123:prod` from the bot identity both fail with rule violations.
...
1. **Ruleset configuration is a manual GitHub UI step.** Not enforced by code. If the operator forgets §6 step 8, agents (or anyone with bot push permissions) can push directly to `prod`. *Mitigation:* `make verify-prod-protection` calls `gh api repos/:owner/:repo/rulesets`, asserts the active branch ruleset for `refs/heads/prod` matches the §3.2 contract clause-by-clause (see Makefile target). Operator runs the verifier post-rollout and after any ruleset edit. Personal repos require rulesets (not classic protection) — see §3.2.
