...
The codex review flagged that an earlier version of this doc was missing the `plugins/cloglog/hooks/agent-shutdown.sh` SessionEnd backstop entirely. Rows #25–#27 were added to close that gap, with two new follow-up tasks (T-474 for the backstop work-log/learnings, scope-expanded T-468 for the unregister POST body) and corrections to rows #2/#11/#12 to match the codebase as it actually is. Pin: `tests/plugins/test_agent_shutdown_artifact_shape.py` (to be added by T-472) prevents the next sweep from re-missing the hook.
...
- **T-468** (inbox JSON event helper, scope now also covers the `unregister-by-path` POST body in `agent-shutdown.sh:215-225` per row #27) — escape safety, but printf works today.
