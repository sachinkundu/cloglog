...
| 14 | All printf-JSON inbox event emitters — the **six** agent-emitted supervisor events enumerated in `plugins/cloglog/docs/agent-lifecycle.md:292-299`: `agent_started` (`templates/AGENT_PROMPT.md:131-133`), `pr_merged_notification` (`skills/github-bot/SKILL.md:191`, `templates/AGENT_PROMPT.md:181-185`), `agent_unregistered`, `mcp_unavailable`, `mcp_tool_error`, and `need_session_restart` (emitted on `mcp_tools_updated` per agent-lifecycle.md §6 / close-wave/SKILL.md:323). Every site builds the JSON line with `printf '{"type":"…",…}' "$(date -Is)"`. | 5–7 | no | no | yes — values may contain shell-special chars in branch/wt names | no | **string.Template** in a tiny helper (`scripts/inbox_event.py emit <event-type> --field key=value …`) — JSON via `json.dumps` removes any escape risk. Jinja2 is overkill for one-line JSON. | S | snapshot test per event type (all six shapes including `need_session_restart`); round-trip parse | T-468 |
...
| 28 | `.cloglog/on-worktree-create.sh:174-175` — repo-local cloglog bootstrap hook builds `_body=$(printf '{"worktree_path":"%s","worktree_name":"%s"}' "${WORKTREE_PATH}" "${WORKTREE_NAME}")` and POSTs it to `/api/v1/agents/close-off-task` (caller pinned by `docs/contracts/d2-close-off-template.openapi.yaml` and `tests/test_on_worktree_create_per_project_credentials.py`) | 2 | no | no | yes — `WORKTREE_PATH` is an absolute filesystem path; on hosts whose checkout lives at e.g. `/Users/x"y/cloglog` the printf string would corrupt | **YES (latent)** — same hand-rolled-JSON class as the unregister POST body | **string.Template + json.dumps** via the same `inbox_event.py` helper introduced by T-468; this hook becomes a `python3 inbox_event.py post …` invocation with `--field worktree_path=... --field worktree_name=...`. Closes the JSON-from-bash class for the entire codebase. | XS | snapshot test for the rendered body; special-chars regression with quoted/escaped path | folded into T-468 |
...
- **T-468** (inbox JSON event helper, scope now also covers `agent_started` in `templates/AGENT_PROMPT.md:131-133`, the `unregister-by-path` POST body in `agent-shutdown.sh:215-225` per row #27, and the `.cloglog/on-worktree-create.sh:174-175` close-off-task POST per row #28) — escape safety, but printf works today.
