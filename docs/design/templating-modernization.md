...
| 14 | All printf-JSON inbox event emitters: `plugins/cloglog/skills/github-bot/SKILL.md:191` (`pr_merged_notification`), `templates/AGENT_PROMPT.md:131-133` (`agent_started`), `templates/AGENT_PROMPT.md:181-185` (`pr_merged_notification` in shutdown), AND the per-event shapes documented in `docs/design/agent-lifecycle.md` §4 (`agent_unregistered`, `mcp_unavailable`, `mcp_tool_error`) — every site builds the JSON line with `printf '{"type":"…",…}' "$(date -Is)"`. | 5–7 | no | no | yes — values may contain shell-special chars in branch/wt names | no | **string.Template** in a tiny helper (`scripts/inbox_event.py emit <event-type> --field key=value …`) — JSON via `json.dumps` removes any escape risk. Jinja2 is overkill for one-line JSON. | S | snapshot test per event type (the six shapes); round-trip parse | T-468 |
