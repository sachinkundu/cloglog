...
- The `_sed_escape_replacement` helper added in this PR uses `sed 's/[&|\]/\\&/g'`. T-354 will inherit the same escape concern — when the static template is materialised via `cp` + `sed`, the same replacement-string metacharacters need escaping. The helper shape is the canonical form; do not reinvent it.
