...
| Surviving launcher | yes — `launch.sh` PID 2122767 + claude PID 2122787 still up after `agent_unregistered`. Closed the tab via `close-zellij-tab.sh` (rc=0); launcher's HUP trap fired at 10:04:13 (`/tmp/agent-shutdown-debug.log`); `_unregister_fallback` hit `/api/v1/agents/unregister-by-path` and got `{"detail":"Worktree not found for path: ..."}` (already unregistered — idempotent). All processes gone after the trap. **Note for follow-up:** the `exit-on-unregister.sh` PostToolUse hook scheduled a TERM at 09:53:35 to claude PID `2187279`, which is NOT the surviving claude PID `2122787` — the hook targeted a stale PID from an earlier session in this worktree. The hook's claim "schedules a TERM to claude on successful unregister, so the launcher's wait returns naturally" did not hold here. File T-NEW: investigate why exit-on-unregister.sh resolved the wrong claude PID across an in-place agent-resume (relaunched in same tab after a previous task's PR merged). |
...
### Routing
- **Surviving-launcher follow-up** → file as backlog task (see Shutdown summary). Not a Step 11 invariant — it's a regression in `exit-on-unregister.sh`'s PID resolution across in-place relaunches.
...
`ReviewLoop.__init__` now accepts six optional kwargs across two features (`session_factory`, `head_branch` from T-374; `ci_dispatcher` from T-377; `session_index` plumbed through call sites from T-375). `ReviewTurnSnapshot` carries six optional fields across three features (`findings_json`/`learnings_json` from T-367; `session_index`/`posted_at` from T-375). Any next task touching these should keep all rationale comments inline — they're how a future reader learns *why* the fields exist.
