...
export function createServer(client: CloglogClient): McpServer {
  ...
  server.tool(
    ...
    wrapHandler(async ({ task_id, status, pr_url, skip_pr }: { task_id: string; status: string; pr_url?: string; skip_pr?: boolean }) => {
      ...
      if (status === 'review' && pr_url) {
        ...
        text += `\n\nCRITICAL — You MUST set up a PR polling loop NOW. Without it you will never know when the PR is merged or has review comments. Run:\n\n/loop 5m Check PR #${prNum} for review comments, CI status, and merge state using the github-bot skill. If new comments: move task to in_progress, address feedback, push fix, move back to review. If merged: call mark_pr_merged with the PR URL, then call report_artifact (for spec/plan tasks), then call get_my_tasks and start the next task.`
      }
      return { content: [{ type: 'text' as const, text }] }
    })
  )
  ...
  server.tool(
    'mark_pr_merged',
    'Mark the task matching this PR URL as pr_merged=True. Call this when the polling loop detects a GitHub PR was merged, as a fallback when the GitHub webhook has not fired. This allows start_task to proceed for the next task.',
    {
      worktree_id: z.string().describe('UUID of the worktree'),
      pr_url: z.string().describe('GitHub PR URL (e.g. https://github.com/owner/repo/pull/42)'),
    },
    async ({ worktree_id, pr_url }) => {
      requireRegistered()
      const result = await handlers.mark_pr_merged({ worktree_id, pr_url })
