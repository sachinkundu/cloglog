...
export interface ToolHandlers {
  ...
  list_worktrees(args: { project_id: string }): Promise<unknown>
...
export function createToolHandlers(client: CloglogClient): ToolHandlers {
  return {
    ...
    async request_shutdown({ worktree_id }) {
      return client.request('POST', `/api/v1/agents/${worktree_id}/request-shutdown`)
    },
    ...
    async list_worktrees({ project_id }) {
      return client.request('GET', `/api/v1/projects/${project_id}/worktrees`)
    },
