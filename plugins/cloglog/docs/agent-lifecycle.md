...
| --- | --- | --- |
| `codex_review_timed_out` | `src/gateway/webhook_consumers.py: emit_codex_review_timed_out` (called from `ReviewEngineConsumer._review_pr` on a terminal-stage codex timeout, AFTER the backend ATTEMPTED to post the AGENT_TIMEOUT skip comment via the codex bot). Carries `pr_url`, `pr_number`, `repo_full_name`, `diff_size` (changed lines), `timeout_seconds` (the budget that was hit), `ts`, and a human-readable `message`. T-374. | Informational. The backend ATTEMPTED to post the AGENT_TIMEOUT skip comment to the PR — but `_post_agent_skip` is best-effort (`src/gateway/review_engine.py`) and `post_skip_comment` swallows GitHub HTTP failures, so the comment may not have landed. If the supervisor sees a clustered or repeated timeout signal with no corresponding GitHub comment, that's the diagnostic clue. The supervisor's job is to surface the timeout to the operator (so a recurring spike on a particular repo gets noticed) and to flag whether `REVIEW_TIMEOUT_*` budgets in `src/gateway/review_engine.py` need retuning. The supervisor MUST NOT auto-retry the review by force-unregistering or relaunching the worktree — codex re-runs naturally on the next `synchronize` webhook (i.e., the next push). |
