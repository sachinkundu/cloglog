...
1. Reviewer's GitHub login appears in `reviewer_bot_logins` of `.cloglog/config.yaml` — the `reviewer` field on the inbox event payload is matched against that list. The cloglog defaults are the codex and opencode reviewer bots; downstream projects ship their own.
