...
If the inbox monitor is not running, events pile up silently in the file and nothing will react to them. Restart it with `Monitor(command="tail -f <WORKTREE_PATH>/.cloglog/inbox", persistent=true)` as soon as you notice — **always use the absolute worktree path**, never `tail -f .cloglog/inbox` (relative). The setup/launch skills' dedupe filter matches on the inbox path suffix, but using the same absolute form everywhere is the simplest way to keep `TaskList` reconciliation unambiguous across `/clear` cycles.
