...
- **`issue_comment`** — a reviewer posted an issue-style PR comment. These are often clarifying questions or approvals without a formal review. Read the body; if it requires a code change, follow the same in_progress → fix → review flow. If it is informational (e.g., "LGTM, just waiting on CI"), reply via the *Reply to Review Comments* section and stay in `review`. **If the comment is operator direction to disregard a prior codex finding** (or otherwise settles a pending codex round without a code change — e.g., "ignore that file", "that's intentional", "old demo, leave it"), reply acknowledging the disregard AND push an empty commit (`git commit --allow-empty -m "noop: re-run codex after operator disregard"` then push as the bot) to re-trigger codex on the new SHA. Codex only re-fires on `synchronize` (push) — a comment alone leaves the previous round's findings as the latest review, which blocks `codex-finalized` from dispatching and prevents `ci.yml` from running. See PR #322 (T-424, 2026-05-04) for the failure mode.
...
5. **Inbox monitor is atomic with PR creation** — creating a PR without an active `Monitor` on your worktree `.cloglog/inbox` means you'll never see review comments, CI failures, or merge notifications. Webhook events arrive there directly — no `/loop` needed. Board update + inbox monitor are both mandatory after every PR. The `enforce-inbox-monitor-after-pr.sh` PostToolUse hook enforces this rule at the harness layer — if you skip the monitor-arm, your next tool call will be blocked with an actionable message naming the exact `Monitor(...)` call to make.
