...
- **Merged PR + task in review**: call `mcp__cloglog__mark_pr_merged` with the task's `pr_url` to flip `pr_merged=True` (unblocks `start_task` guard); then flag for user — only the user can move the task to done
