...
3. **If backlog tasks remain** → relaunch in the same zellij tab using the continuation prompt:
   ...
   ```
   The new session reads the prior work logs (see worktree-agent **Work-Log Bootstrap**), **calls `register_agent` to bind its new MCP session** (the previous session's `unregister_agent` cleared its per-process state), and starts the next backlog task by following Standard workflow step 3 in `${CLAUDE_PLUGIN_ROOT}/templates/AGENT_PROMPT.md` unchanged (trust `task.md`, escalate on backend error). The supervisor MUST rewrite `${WORKTREE_PATH}/task.md` for the next task before issuing this continuation prompt — see the Continuation Prompt section of `plugins/cloglog/skills/launch/SKILL.md`.
4. **Confirmation phase — same `agent_started` deadline as initial launch (T-356).** A continuation prompt can trip the same bootstrap failures as an initial launch (claude crash, heredoc-rendered `launch.sh` with a syntax error, MCP unavailable, invalid `--model` for the next task). After issuing the relaunch keystrokes, watch `<project_root>/.cloglog/inbox` for an `agent_started` event whose `worktree` field equals `${WORKTREE_NAME}`, with deadline `launch_confirm_timeout_seconds` from `.cloglog/config.yaml` (default 90s). On timeout, emit the same five-item diagnostic checklist as the launch SKILL's Step 5 (`zellij action list-tabs --json | jq`, `bash -n <worktree>/.cloglog/launch.sh`, `tail -20 /tmp/agent-shutdown-debug.log`, separately verify `CLOGLOG_API_KEY` via `printenv` or `~/.cloglog/credentials.d/<project_slug>` (for repos with `project_id` in `config.yaml`) / `~/.cloglog/credentials` (legacy — only when no `project_id`) and `DATABASE_URL` via `<worktree>/.env`, `head -3 <worktree>/.cloglog/launch.sh`) and hand off to the operator. **Do NOT silently retry; do NOT loop the relaunch up to N times.** The supervisor must NOT mark the worktree as continuing on the next task until either `agent_started` arrives or the operator has acted. This mirrors the launch SKILL's *Supervisor Relaunch Flow* exactly — both call sites enforce the same contract.
5. **If no backlog tasks remain** → the worktree is done. Invoke `/cloglog close-wave <wt-name>` to run the cooperative shutdown, consolidate artifacts, and tear down the worktree.
...
- **`mcp_unavailable`** — the agent could not reach MCP at startup (ToolSearch returned no matches, or the first post-register call failed at the transport layer). The agent has already exited. Diagnose the outage (is the backend up? was the worktree's `.mcp.json` written? did `CLOGLOG_API_KEY` land in `~/.cloglog/credentials.d/<project_slug>` for repos with `project_id` in `config.yaml`, or `~/.cloglog/credentials` for legacy repos without it?) and, once repaired, `force_unregister` the dead session and relaunch the agent.
