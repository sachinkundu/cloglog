...
- **Agents communicate via inbox files.** Each agent has an inbox at `<worktree_path>/.cloglog/inbox` — the per-worktree file the webhook consumer and backend both write to. See `plugins/cloglog/docs/agent-lifecycle.md` Section 3 for the inbox contract and a note on the removed legacy path.
...
- **Halt on any MCP failure: startup unavailability emits `mcp_unavailable` and exits; runtime tool errors emit `mcp_tool_error` and wait for the main agent; transient network errors get one backoff retry before escalating.** See `plugins/cloglog/docs/agent-lifecycle.md` §4.1 for the full rule and both event shapes.
