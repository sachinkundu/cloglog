...
- **Every phase of a feature needs a board task.** The full pipeline:
  ...
  2. **"Write implementation plan for F-N"** — no PR needed. Write the plan, commit it locally. Then call `update_task_status(plan_task_id, "review", skip_pr=True)` followed by `report_artifact(plan_task_id, worktree_id, plan_path)` — without both calls, the pipeline guard blocks the impl task. No separate approval needed; proceed to implementation immediately after.
