...
async def test_agent_sees_only_own_tasks(client: AsyncClient) -> None:
    ...
    # A sees only task 1
    resp_a = await client.get(f"/api/v1/agents/{agent_a.worktree_id}/tasks")
    ...
    # B sees only task 2
    resp_b = await client.get(f"/api/v1/agents/{agent_b.worktree_id}/tasks")
...
async def test_unregister_one_leaves_other_active(client: AsyncClient) -> None:
    ...
    # Unregister A
    unreg = await client.post(f"/api/v1/agents/{agent_a.worktree_id}/unregister")
    assert unreg.status_code == 204

    # B's heartbeat still works
    hb = await client.post(f"/api/v1/agents/{agent_b.worktree_id}/heartbeat")
...
async def test_messages_isolated_between_agents(client: AsyncClient) -> None:
    ...
    hb_b = await client.post(f"/api/v1/agents/{agent_b.worktree_id}/heartbeat")
    assert hb_b.status_code == 200
    assert hb_b.json()["pending_messages"] == []

    # A's heartbeat returns the message
    hb_a = await client.post(f"/api/v1/agents/{agent_a.worktree_id}/heartbeat")
...
async def test_concurrent_heartbeats(client: AsyncClient) -> None:
    ...
    hb_a, hb_b = await asyncio.gather(
        client.post(f"/api/v1/agents/{agent_a.worktree_id}/heartbeat"),
        client.post(f"/api/v1/agents/{agent_b.worktree_id}/heartbeat"),
    )
    assert hb_a.status_code == 200
    assert hb_b.status_code == 200
