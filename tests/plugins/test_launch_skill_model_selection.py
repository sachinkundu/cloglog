...
def test_launch_skill_writes_task_model_file_before_launch_sh() -> None:
    """Step 4e must write .cloglog/task-model before rendering launch.sh.

    Post-T-354 the rendering uses ``render_template.py`` instead of an
    inline ``cat > ... << EOF`` heredoc. The ordering invariant is the
    same: the per-task model file must exist before launch.sh runs.
    """
    ...
    render_call_pos = section.find("render_template.py")
    assert task_model_write_pos != -1, (
        "Step 4e must write the task's model to .cloglog/task-model before "
        "rendering launch.sh (T-332). Add: "
        'printf \'%s\\n\' "${TASK_MODEL:-}" > "${WORKTREE_PATH}/.cloglog/task-model"'
    )
    assert render_call_pos != -1, (
        "Step 4e must invoke scripts/render_template.py to render launch.sh (T-354)."
    )
    assert task_model_write_pos < render_call_pos, (
        ".cloglog/task-model write must appear BEFORE the render_template.py "
        "call in Step 4e — launch.sh reads the file on first run."
    )


def test_launch_sh_template_reads_task_model_file() -> None:
    """The launch.sh template must read .cloglog/task-model at runtime.

    Post-T-354 the template lives at ``templates/launch.sh.template``,
    not inside a SKILL heredoc. This allows the supervisor to update the
    file before each continuation relaunch so the correct model is used
    for every task in the worktree.
    """
...
def test_launch_sh_template_claude_invocation_uses_model_flag() -> None:
    ...
    The form ``${_MODEL_FLAG:+$_MODEL_FLAG}`` means:
    ...
    assert re.search(r"claude --dangerously-skip-permissions.*_MODEL_FLAG", body), (
        "The claude invocation in launch.sh.template must reference _MODEL_FLAG "
